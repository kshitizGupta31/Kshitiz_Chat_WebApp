...
//       justify-center p-5 focus:border-none hover:bg-[#741bda]
//        focus:bg-[#741bda]   focus:outline-none focus:text-white duration-300
//        transition-all"
...
function MessageBar() {
  ...
  const handleSendMessage = async () => {
    if (selectedChatType === "contact") {
      socket.emit("sendMessage", {
        sender: userInfo.id,
        content: message, // Changed `Content` to `content` to follow camelCase convention
        recipient: selectedChatData._id,
        messageType: "text",
        fileUrl: undefined,
      });
      ...
      if (file) {
        ...
        if (response.status === 200 && response.data) {
          ...
          if (selectedChatType === "contact") {
            socket.emit("sendMessage", {
              sender: userInfo.id,
              content: undefined, // Changed `Content` to `content` to follow camelCase convention
              recipient: selectedChatData._id,
              messageType: "file",
